...
int main(int argc, char *argv[])
{
   ...
   // pusher.add_collider(frowneyC, frowneyNode);
    std::cout << "x" << window->get_graphics_window()->get_pointer(0).get_x() << std::endl;
   ...
   std::cout << "x" << window->get_graphics_window()->get_pointer(0).get_x() << std::endl; // WORKING

   if(!mouseNode.is_empty())
      {
         MouseWatcher* mouseWatcher = dynamic_cast<MouseWatcher*>(mouseNode.node()); // NOT WORKING

         std::cout << "mouseWatcher->has_mouse()" << mouseWatcher->has_mouse() << std::endl;
